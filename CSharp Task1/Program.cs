...
class Program
{
    ...
    static void Main()
    {
        ...
        {
            //debtors.FindAll((debtor) => DateTime.Today.Year - debtor.BirthDay.Year > 26 && DateTime.Today.Year - debtor.BirthDay.Year < 36).ForEach((debtor)=>Console.WriteLine(debtor));
        }
        //4) Borcu 5000 - den cox olmayan borclularic cixartmag
        {
            //debtors.FindAll((debtor) => debtor.Debt <= 5000).ForEach((debtor)=>Console.WriteLine(debtor));
        }
        //5) Butov adi 18 simvoldan cox olan ve telefon nomresinde 2 ve ya 2 - den cox 7 reqemi olan borclulari cixartmaq
        {
            //debtors.FindAll((debtor) => debtor.FullName.Length > 18 && debtor.Phone.Count((chr)=>chr == '7') >= 2).ForEach((debtor)=>Console.WriteLine(debtor));
        }
        //7) Qishda anadan olan borclulari cixardmaq
        {
            //debtors.FindAll((debtor) => debtor.BirthDay.Month == 12 || debtor.BirthDay.Month == 1 || debtor.BirthDay.Month == 2).ForEach((debtor) => Console.WriteLine(debtor));
        }
